...
final class KeyAll extends Timer {

	/** @var string[] */
	private array $commands = [];

	public function __construct() {
		parent::__construct('Key All', 'Use timer to key all', '&l&9Key All ends in &r&7:', 60 * 60);
	}
	...
	public function setEnabled(bool $enabled) : void {
		...
		if ($enabled) {
			HCF::getInstance()->getServer()->getCommandMap()->register('HCF', new class($this) extends Command {
				...
				public function __construct(
					private KeyAll $manager
				) {
					parent::__construct('keyall', 'Use command to key all');
					$this->setPermission('key_all.command');
				}
				...
				public function execute(CommandSender $sender, string $commandLabel, array $args) : void {
					...
					switch (strtolower($args[0])) {
						case 'help':
							$text = [
								'&l&4Key All Commands&r',
								'&4/keyall add &7- Use command to add command',
								'&4/keyall remove &7- Use command to remove command',
								'&4/keyall list &7- Use command to view list'
							];
						...
						case 'add':
							if (count($args) < 2) {
								$sender->sendMessage(TextFormat::colorize('&cUse /keyall add [command]'));
								return;
							}
							$command = $args[1];
						...
						case 'remove':
							if (count($args) < 2) {
								$sender->sendMessage(TextFormat::colorize('&cUse /keyall remove [position]'));
								return;
							}
							$position = (int) $args[1];
			...
			$command = HCF::getInstance()->getServer()->getCommandMap()->getCommand('keyall');
